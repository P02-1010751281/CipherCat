...
\begin{frontmatter}
	...
	\begin{abstract}
		CipherCat 是一个面向后量子密码学（PQC）的开源可视化编程平台，基于 Blockly 和 Vue 3 构建。平台将 FIPS 203 (ML-KEM) 标准中的 CBD 采样、NTT/INTT 变换、编码压缩、ML-KEM 复合运算等 17 个 PQC 专用原语封装为可拖拽的积木块，用户通过组合积木块构建密码算法工作流，系统自动生成可独立运行的 Python 或 JavaScript 代码。CipherCat 旨在降低 PQC 算法的学习与原型开发门槛，服务于密码学教育、算法验证和工程原型设计等场景。
	\end{abstract}
...
然而，PQC 算法的学习与开发存在显著门槛。以 ML-KEM 为例，其核心涉及 $\mathbb{Z}_q[X]/(X^{256}+1)$ 上的多项式环运算、Cooley-Tukey / Gentleman-Sande 蝶形 NTT/INTT 变换、中心二项分布（CBD）采样、系数压缩与字节编码等十余种数学原语。传统方式依赖阅读 FIPS 规范文档并手工编码实现——学习者不仅需要扎实的抽象代数和数论基础，底层实现细节（如 NTT 蝶形运算的位逆序索引、$\mathbb{Z}_q$ 上的模乘边界处理）也极易引入难以调试的错误。已有研究证实，密码学 API 的误用和实现缺陷是安全漏洞的重要来源 \cite{nadi2016,mouha2018,hazhirpasand2021}。

这种"复杂数学 → 手动编码"的门槛问题并不限于密码学领域。在深度学习领域，VELCRO \cite{lim2024velcro} 通过可视化积木块降低了 DNN 架构构建的认知负担——这表明将抽象数学结构转化为图形化原语是一种经过验证的降低门槛策略。密码学领域同样需要这样一种工具。
...
ibitem{metacrypt2024}
		肖超恩, 刘昌俊, 董秀则, 王建新, 张磊. 基于模型驱动的密码算法可视化开发平台研究. 密码学报(中英文) 2024;11(02):357--370. https://doi.org/10.13868/j.cnki.jcr.000684

	\bibitem{fips180}
	National Institute of Standards and Technology. Secure Hash Standard (SHS). NIST FIPS 180-4; 2015. https://csrc.nist.gov/pubs/fips/180-4/upd1/final

	\bibitem{sm3}
	Standardization Administration of China. Information security technology — SM3 cryptographic hash algorithm. GB/T 32905-2016; 2016.
	...
	\bibitem{fips202}
	National Institute of Standards and Technology. FIPS 202: SHA-3 Standard: Permutation-Based Hash and Extendable-Output Functions. NIST FIPS 202; 2015. https://csrc.nist.gov/pubs/fips/202/final
	...
	\bibitem{stebila2017}
	Stebila D, Mosca M. Post-quantum key exchange for the Internet and the Open Quantum Safe project. In: Selected Areas in Cryptography (SAC) 2016, LNCS, vol. 10532, Springer, 2017, p. 1--24. https://doi.org/10.1007/978-3-319-69453-5\_1
