  ...
  init: function () {
    ...
    this.setTooltip(
      Blockly.Msg.CRYPTO_XOF_TOOLTIP ||
        'XOF: Extendable Output Function (SHAKE128/SHAKE256). Generates arbitrary-length pseudorandom byte stream from a seed (FIPS 202).',
    );
  ...
  init: function () {
    ...
    this.setTooltip(
      Blockly.Msg.CRYPTO_PRF_TOOLTIP ||
        'PRF: Pseudo-Random Function (SHAKE128/SHAKE256). seed || nonce → outLen bytes. Used by CBD sampling (FIPS 203, Algorithm 8).',
    );
    this.setHelpUrl('https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.202.pdf');
  },
};
