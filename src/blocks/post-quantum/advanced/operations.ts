  ...
  init: function () {
    ...
    this.setTooltip(
      'SampleNTTMat(seed, k, q): Generate k×k SampleNTT matrix A from seed ρ. ' +
        'For i,j in range(k): A[i][j] = SampleNTT(seed || j || i). ' +
...
Blockly.Blocks['pq_build_vec3'] = {
  init: function () {
    this.appendValueInput('A').setCheck(null).appendField('BuildVec₃(');
...
Blockly.Blocks['pq_vec_compress_encode'] = {
  init: function () {
    this.appendValueInput('U').setCheck(null).appendField('VecCompressEncode(');
    this.appendDummyInput()
      .appendField('d=')
      .appendField(
        new Blockly.FieldDropdown([
