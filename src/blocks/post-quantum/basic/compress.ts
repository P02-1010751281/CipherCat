...
export const COMPRESS_BLOCK_TYPES = [
  'pq_compress',
  'pq_decompress'
] as const;
...
Blockly.Blocks['pq_compress'] = {
  init: function() {
    ...
    this.setColour(230);
    this.setTooltip('Compress_q(x, d): Compress integer(s) modulo q to d bits. Compress(x,d) = round((2^d/q)·x) mod 2^d (FIPS 203)');
...
Blockly.Blocks['pq_decompress'] = {
  init: function() {
    ...
    this.setOutput(true, null);
    this.setColour(230);
    this.setTooltip('Decompress_q(y, d): Decompress d-bit integer(s) back to modulo q. Decompress(y,d) = round((q/2^d)·y) (FIPS 203)');
    this.setHelpUrl('https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.203.pdf');
  },
};
